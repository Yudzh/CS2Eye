...
export interface MatchSeries {
  ...
  veto: VetoAction[];
  round_number:number|null;round_label:string|null;group_name:string|null;bracket_section:"main"|"upper"|"lower"|"group"|"swiss"|null;bracket_position:number|null;next_match_id:number|null;next_match_slot:"team_a"|"team_b"|null;
}
...
export interface LLMMatchAnalysisContext{schema_version:"match_analysis_context.v1";generated_at:string;as_of:string;analysis_mode:"pre_match"|"post_match";match:{id:number|null;date:string|null;format:"bo1"|"bo3"|"bo5"|null;environment:"lan"|"online"|null;stage:string|null;is_playoff:boolean|null;is_elimination:boolean|null;tournament:{id:number|null;name:string|null;tier:string|null};round_number:number|null;round_label:string|null;section:string|null};teams:{team_a:LLMTeamContext;team_b:LLMTeamContext};recent_series_evidence:{team_a:LLMRecentSeries[];team_b:LLMRecentSeries[]};prediction:{source_type:"ml_prediction";status:string;model_version:string|null;quality_gate_passed:boolean|null;team_a_probability:number|null;team_b_probability:number|null;confidence:number|null;top_model_drivers:Array<{driver_id:string;key:string;favored_team:LLMSide|null;importance:number|null;reliability:number|null}>};matchup:{source_type:"deterministic_analytics";model_version:string|null;team_a_score:number|null;team_b_score:number|null;reliability:number|null;factors:Array<{factor_id:string;key:string;team_a_score:number|null;team_b_score:number|null;reliability:number|null;sample_size:number|null}>};veto:{source_type:"deterministic_analytics";basis:"calculated_veto";model_version:string|null;likely_maps:Array<{map:string;series_probability:number|null;confidence:number|null;likely_role:string}>};map_matchups:Array<{map:string;relevance:number|null;team_a:{map_strength:number|null;reliability:number|null;sample_maps:number};team_b:{map_strength:number|null;reliability:number|null;sample_maps:number};matchup_score_team_a:number|null;key_edges:Array<{evidence_id:string;metric:string;favored_team:"team_a"|"team_b";strength:string;reliability:number|null}>}>;h2h:{preferred_scope:string;history_applicability:string;organizations:Record<string,unknown>;current_rosters:Record<string,unknown>};manual_context:{source_type:"manual_analyst_note";team_a:LLMManualNote[];team_b:LLMManualNote[]};data_quality:{overall_status:"available"|"partial"|"weak"|"insufficient";limitations:string[];missing_sections:string[];warnings:string[]}}
...
export type MatchLLMAnalysis=MatchLLMAnalysisV1|MatchLLMAnalysisV2;
...
export interface MatchLLMAnalysisRun{analysis_run_id:number;source_run_id:number|null;status:"pending"|"completed"|"failed"|"skipped_insufficient_data";context:LLMMatchAnalysisContext;explanation_plan?:MatchExplanationPlan|null;analysis:MatchLLMAnalysis|null;runtime:MatchLLMRuntime;error_code:string|null;error_message:string|null;validation_error_codes:string[];created_at:string;completed_at:string|null}
...
export interface MatchLLMGenerateRequest{team_a_id:number;team_b_id:number;as_of:string;match_id?:number;tournament_id?:number;analysis_mode:"pre_match";language:"ru"}
