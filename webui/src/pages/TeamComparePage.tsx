...
import type { CalculatedVeto, CalculatedVetoSide, MapConfidenceLevel, MatchupScore, Team, TeamComparison, TeamH2HComparison, TeamH2HRosterContext, TeamH2HSlice, TeamMapComparisonItem, TeamMapComparisonResponse, VetoComparison, WinProbability } from "../types";
...
const factorLabels:Record<string,string>={own_map_quality:"Собственная сила карты",relative_advantage:"Преимущество над соперником",recent_roster_form:"Форма текущего состава",side_matchup:"Сопоставление сторон",bomb_matchup:"Постплэнт и ретейк",economy_combat_matchup:"Экономика и боевые действия",utility_teamplay_matchup:"Гранаты и командная игра",matchup_map_score:"Оценка противостояния",historical_pick_preference:"История выборов",recent_veto_preference:"Недавнее вето",own_weakness:"Слабость команды",opponent_map_threat:"Угроза от соперника",relative_disadvantage:"Относительный недостаток",historical_ban_preference:"История запретов"};
function CalculatedSide({side}:{side:CalculatedVetoSide}) { const explanations=[...side.pick_factors,...side.ban_factors].filter(f=>f.available).sort((a,b)=>Math.abs(b.impact)-Math.abs(a.impact)).slice(0,3);return <span><b><Term tip="Комплексная оценка того, насколько карта подходит команде именно против выбранного соперника.">Соответствие {side.matchup_map_score.toFixed(1)}</Term></b><small>Выбор {side.calculated_pick_score.toFixed(1)}/100 · Запрет {side.calculated_ban_score.toFixed(1)}/100</small><details><summary>Почему</summary>{explanations.map(f=><small key={`${f.key}-${f.weight}`}>{f.impact>=0?"+":""}{f.impact.toFixed(1)} · {factorLabels[f.key]??f.label} ({f.score?.toFixed(1)??"—"}, вес {(f.effective_weight*100).toFixed(0)}%)</small>)}</details></span> }
  ...
  return (
    <main className="page compare-page">
      ...
      {loadingComparison ? <div className="empty-state">Собираю сравнение команд…</div> : comparison ? <>
        ...
        <section className="map-pool-comparison"><div className="section-heading"><div><p className="eyebrow">Фактические данные</p><h2><Term tip="Реальные запреты и выборы карт из сохранённых серий. Это история действий команд, а не рекомендация модели.">Фактическое вето</Term></h2></div></div>{!veto||veto.team_a.sample.series===0||veto.team_b.sample.series===0?<div className="empty-state">Недостаточно истории фактического вето.</div>:<div className="map-pool-table"><div className="map-pool-row map-pool-row--head"><span>Карта</span><span>{veto.team_a.team_name}</span><span>{veto.team_b.team_name}</span><span><Term tip="Насколько выбор карты одной командой сталкивается с желанием соперника её запретить.">Столкновение</Term></span><span>Доступность</span></div>{veto.maps.filter(m=>m.team_a.active||m.team_b.active).map(m=><div className="map-pool-row" key={m.map_name}><strong>{m.map_name}</strong><span>Выбор {m.team_a.pick.rate?.toFixed(1)??"—"}% · Запрет {m.team_a.ban.rate?.toFixed(1)??"—"}%</span><span>Выбор {m.team_b.pick.rate?.toFixed(1)??"—"}% · Запрет {m.team_b.ban.rate?.toFixed(1)??"—"}%</span><span>{collisionLabels[m.collision]??m.collision}</span><span>{availabilityLabels[m.availability]??m.availability.replaceAll("_"," ")}</span></div>)}</div>}<p className="formula">Только сохранённые реальные выборы и запреты. Очная история вето: {veto?.h2h.series??0} последних серий.</p></section>
        <section className="map-pool-comparison calculated-veto"><div className="section-heading"><div><p className="eyebrow">Расчёт CS2Eye · не официальное вето</p><h2><Term tip="Модельная оценка логичных запретов и выборов карт на основе силы карт, формы, сторон, экономики, боя, Swing и истории вето.">Расчётное вето</Term></h2></div><span>версия модели {calculatedVeto?.calculated_veto_model_version??"—"}</span></div>{!calculatedVeto?<div className="empty-state">Расчёт недоступен.</div>:<><div className="map-pool-table"><div className="map-pool-row map-pool-row--head"><span>Карта</span><span>{calculatedVeto.team_a.name}</span><span>{calculatedVeto.team_b.name}</span><span><Term tip="Сочетание желания одной стороны выбрать карту и желания соперника её запретить.">Столкновение</Term></span><span><Term tip="Надёжность с учётом объёма, полноты и свежести данных обеих команд.">Надёжность</Term></span></div>{calculatedVeto.maps.map(m=><div className="map-pool-row" key={m.map}><strong>{m.map}</strong><CalculatedSide side={m.team_a}/><CalculatedSide side={m.team_b}/><span>{collisionLabels[m.collision]??m.collision} · {m.collision_score.toFixed(1)}/100</span><span>{(m.team_a.matchup_confidence*100).toFixed(0)} / {(m.team_b.matchup_confidence*100).toFixed(0)}</span></div>)}</div><div className="h2h-grid">{calculatedVeto.scenarios.map(s=><article className="h2h-card" key={s.first_actor}><h3>Сценарий: первым действует {s.first_actor==="team_a"?calculatedVeto.team_a.name:calculatedVeto.team_b.name}</h3><ol>{s.actions.map(a=><li key={a.order}>{a.team===null?"Решающая карта":a.team==="team_a"?calculatedVeto.team_a.name:calculatedVeto.team_b.name} — {a.action==="pick"?"выбирает":a.action==="ban"?"запрещает":a.action} <b>{a.map}</b>{a.effective_action_score!==null&&` · оценка ${a.effective_action_score.toFixed(1)}/100`}</li>)}</ol></article>)}</div><p className="formula">Оценки выбора и запрета имеют шкалу 0–100 и не являются вероятностями. Используется стандартный порядок BO3; если первая действующая команда неизвестна, показаны два сценария.</p></>}</section>
