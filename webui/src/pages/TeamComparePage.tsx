...
import { compareTeamMaps, compareTeams, compareTeamVeto, getCalculatedVeto, getTeamH2H, getTeams } from "../api";
...
import type { CalculatedVeto, CalculatedVetoSide, MapConfidenceLevel, Team, TeamComparison, TeamH2HComparison, TeamH2HRosterContext, TeamH2HSlice, TeamMapComparisonItem, TeamMapComparisonResponse, VetoComparison } from "../types";
...
function CalculatedSide({side}:{side:CalculatedVetoSide}) { const explanations=[...side.pick_factors,...side.ban_factors].filter(f=>f.available).sort((a,b)=>Math.abs(b.impact)-Math.abs(a.impact)).slice(0,3);return <span><b>Matchup {side.matchup_map_score.toFixed(1)}</b><small>Pick {side.calculated_pick_score.toFixed(1)}/100 · Ban {side.calculated_ban_score.toFixed(1)}/100</small><details><summary>Почему</summary>{explanations.map(f=><small key={`${f.key}-${f.weight}`}>{f.impact>=0?"+":""}{f.impact.toFixed(1)} · {f.label} ({f.score?.toFixed(1)??"—"}, вес {(f.effective_weight*100).toFixed(0)}%)</small>)}</details></span> }
...
export function TeamComparePage() {
  ...
  async function loadComparison(a: number, b: number) {
    ...
    try {
      const [organization, maps, vetoData, calculated] = await Promise.all([compareTeams(a, b), compareTeamMaps(a, b), compareTeamVeto(a,b), getCalculatedVeto(a,b)]);
      setComparison(organization); setMapPool(maps); setVeto(vetoData); setCalculatedVeto(calculated);
    }
  ...
  return (
    <main className="page compare-page">
      ...
      {loadingComparison ? <div className="empty-state">Собираю сравнение команд…</div> : comparison ? <>
        ...
        <section className="map-pool-comparison"><div className="section-heading"><div><p className="eyebrow">Current roster · probability impact</p><h2>Round Swing</h2></div></div><div className="map-pool-table"><div className="map-pool-row map-pool-row--head"><span>Показатель</span><span>{comparison.team_a.name}</span><span>{comparison.team_b.name}</span><span>Sample A</span><span>Sample B</span></div>{([['Avg Swing','avg_swing'],['Top-2 Swing','top2_swing'],['Bottom-2 Swing','bottom2_swing'],['T Swing','t_swing'],['CT Swing','ct_swing'],['Opening Swing','opening_swing'],['Clutch Swing','clutch_swing']] as const).map(([label,key])=><div className="map-pool-row" key={key}><strong>{label}</strong><span>{comparison.round_swing_comparison.team_a[key]?.toFixed(2)??'—'}</span><span>{comparison.round_swing_comparison.team_b[key]?.toFixed(2)??'—'}</span><span>{comparison.round_swing_comparison.team_a.sample.rounds} r · {(comparison.round_swing_comparison.team_a.confidence*100).toFixed(0)}%</span><span>{comparison.round_swing_comparison.team_b.sample.rounds} r · {(comparison.round_swing_comparison.team_b.confidence*100).toFixed(0)}%</span></div>)}</div><p className="formula">Профиль только текущих пятёрок; contextual values — subsets общего Swing.</p></section>
        <section className="map-pool-comparison"><div className="section-heading"><div><p className="eyebrow">Standalone analytics</p><h2>Leadership comparison</h2></div></div><div className="map-pool-table"><div className="map-pool-row map-pool-row--head"><span>Показатель</span><span>{comparison.team_a.name}</span><span>{comparison.team_b.name}</span><span>Model A</span><span>Model B</span></div><div className="map-pool-row"><strong>IGL Strength</strong><span>{comparison.team_a.leadership.igl?.score.toFixed(1)??"—"}</span><span>{comparison.team_b.leadership.igl?.score.toFixed(1)??"—"}</span><span>{comparison.team_a.leadership.igl?.model_version??"—"}</span><span>{comparison.team_b.leadership.igl?.model_version??"—"}</span></div><div className="map-pool-row"><strong>Coach Impact</strong><span>{comparison.team_a.leadership.coach?.score.toFixed(1)??"—"}</span><span>{comparison.team_b.leadership.coach?.score.toFixed(1)??"—"}</span><span>{comparison.team_a.leadership.coach?.model_version??"—"}</span><span>{comparison.team_b.leadership.coach?.model_version??"—"}</span></div></div><p className="formula">Leadership не включён в Team Strength или Matchup Score.</p></section>
        <section className="map-pool-comparison"><div className="section-heading"><div><p className="eyebrow">ФАКТИЧЕСКИЕ ДАННЫЕ</p><h2>Фактический veto</h2></div></div>{!veto||veto.team_a.sample.series===0||veto.team_b.sample.series===0?<div className="empty-state">Недостаточно фактической veto-истории.</div>:<div className="map-pool-table"><div className="map-pool-row map-pool-row--head"><span>Карта</span><span>{veto.team_a.team_name}</span><span>{veto.team_b.team_name}</span><span>Collision</span><span>Availability</span></div>{veto.maps.filter(m=>m.team_a.active||m.team_b.active).map(m=><div className="map-pool-row" key={m.map_name}><strong>{m.map_name}</strong><span>Pick {m.team_a.pick.rate?.toFixed(1)??"—"}% · Ban {m.team_a.ban.rate?.toFixed(1)??"—"}%</span><span>Pick {m.team_b.pick.rate?.toFixed(1)??"—"}% · Ban {m.team_b.ban.rate?.toFixed(1)??"—"}%</span><span>{m.collision}</span><span>{m.availability.replaceAll("_"," ")}</span></div>)}</div>}<p className="formula">Только сохранённые реальные picks/bans. H2H veto: {veto?.h2h.series??0} последних серий.</p></section>
        <section className="map-pool-comparison calculated-veto"><div className="section-heading"><div><p className="eyebrow">РАСЧЁТ CS2EYE · НЕ ОФИЦИАЛЬНЫЙ VETO</p><h2>Расчётный veto</h2></div><span>model {calculatedVeto?.calculated_veto_model_version??"—"}</span></div>{!calculatedVeto?<div className="empty-state">Расчёт недоступен.</div>:<><div className="map-pool-table"><div className="map-pool-row map-pool-row--head"><span>Карта</span><span>{calculatedVeto.team_a.name}</span><span>{calculatedVeto.team_b.name}</span><span>Collision</span><span>Confidence</span></div>{calculatedVeto.maps.map(m=><div className="map-pool-row" key={m.map}><strong>{m.map}</strong><CalculatedSide side={m.team_a}/><CalculatedSide side={m.team_b}/><span>{m.collision} · {m.collision_score.toFixed(1)}/100</span><span>{(m.team_a.matchup_confidence*100).toFixed(0)} / {(m.team_b.matchup_confidence*100).toFixed(0)}</span></div>)}</div><div className="h2h-grid">{calculatedVeto.scenarios.map(s=><article className="h2h-card" key={s.first_actor}><h3>Сценарий: первым действует {s.first_actor==="team_a"?calculatedVeto.team_a.name:calculatedVeto.team_b.name}</h3><ol>{s.actions.map(a=><li key={a.order}>{a.team===null?"Decider":a.team==="team_a"?calculatedVeto.team_a.name:calculatedVeto.team_b.name} — {a.action} <b>{a.map}</b>{a.effective_action_score!==null&&` · score ${a.effective_action_score.toFixed(1)}/100`}</li>)}</ol></article>)}</div><p className="formula">Pick/Ban — аналитические scores 0–100, не вероятности. Порядок: {calculatedVeto.veto_format_assumption}; неизвестный first actor показан двумя сценариями.</p></>}</section>
          ...
          {mapPoolLoading ? <div className="empty-state">Сравниваю карты…</div> : mapPoolError ? <div className="empty-state empty-state--error">{mapPoolError}</div> : !mapPool ? <div className="empty-state">Map pool ещё не загружен.</div> : mapPool.maps.length === 0 ? <div className="empty-state">{mapPool.status === "current_roster_unavailable" ? "Текущий состав одной из команд не определён." : mapPoolLevel === "current_roster" ? "У текущих составов пока нет карт." : "Для организаций пока нет агрегатов карт."}</div> : <>
            <div className="map-pool-table">
              ...
              {mapPool.maps.map((item) => <div className="map-pool-row" key={item.map_name}>
                ...
                <span>{item.team_a?.map_strength_score === null || !item.team_a ? "—" : item.team_a.map_strength_score.toFixed(2)}<small>{item.team_a ? `${item.team_a.maps_won}–${item.team_a.maps_lost} · ${item.team_a.maps_played} карт` : "Нет данных"}</small>{item.team_a && <small>Установка {numberLabel(item.team_a.bomb.plant_rate, "%")} · Постплэнт {numberLabel(item.team_a.bomb.postplant_win_rate, "%")} · Ретейк {numberLabel(item.team_a.bomb.retake_win_rate, "%")}</small>}{item.team_a?.economy && <small>Пистолетные {numberLabel(item.team_a.economy.pistol.win_rate, "%")} · Конверсия {numberLabel(item.team_a.economy.conversion.win_rate, "%")} · Форс {numberLabel(item.team_a.economy.force_buy.win_rate, "%")} · Полный {numberLabel(item.team_a.economy.full_buy.win_rate, "%")} · Анти-эко {numberLabel(item.team_a.economy.anti_eco.win_rate, "%")}</small>}{item.team_a?.combat && <small>Opening {numberLabel(item.team_a.combat.opening.success_rate, "%")} · Conversion {numberLabel(item.team_a.combat.opening.conversion_rate, "%")} · Recovery {numberLabel(item.team_a.combat.opening.recovery_rate, "%")} · Trade {numberLabel(item.team_a.combat.trade.trade_rate, "%")} · Clutch {numberLabel(item.team_a.combat.clutch.win_rate, "%")}</small>}{item.team_a?.utility && <small>Utility dmg/round {numberLabel(item.team_a.utility.utility_damage_per_round)} · HE {numberLabel(item.team_a.utility.he_damage_per_round)} · Fire {numberLabel(item.team_a.utility.fire_damage_per_round)} · Enemies/flash {numberLabel(item.team_a.utility.enemies_flashed_per_flash)} · FA/round {numberLabel(item.team_a.utility.flash_assists_per_round)} · Utility/round {numberLabel(item.team_a.utility.utility_per_round)}</small>}{item.team_a && <em className={`confidence-badge confidence-badge--${item.team_a.confidence_level}`}>{confidenceLabels[item.team_a.confidence_level]}</em>}</span>
                <span>{item.team_b?.map_strength_score === null || !item.team_b ? "—" : item.team_b.map_strength_score.toFixed(2)}<small>{item.team_b ? `${item.team_b.maps_won}–${item.team_b.maps_lost} · ${item.team_b.maps_played} карт` : "Нет данных"}</small>{item.team_b && <small>Установка {numberLabel(item.team_b.bomb.plant_rate, "%")} · Постплэнт {numberLabel(item.team_b.bomb.postplant_win_rate, "%")} · Ретейк {numberLabel(item.team_b.bomb.retake_win_rate, "%")}</small>}{item.team_b?.economy && <small>Пистолетные {numberLabel(item.team_b.economy.pistol.win_rate, "%")} · Конверсия {numberLabel(item.team_b.economy.conversion.win_rate, "%")} · Форс {numberLabel(item.team_b.economy.force_buy.win_rate, "%")} · Полный {numberLabel(item.team_b.economy.full_buy.win_rate, "%")} · Анти-эко {numberLabel(item.team_b.economy.anti_eco.win_rate, "%")}</small>}{item.team_b?.combat && <small>Opening {numberLabel(item.team_b.combat.opening.success_rate, "%")} · Conversion {numberLabel(item.team_b.combat.opening.conversion_rate, "%")} · Recovery {numberLabel(item.team_b.combat.opening.recovery_rate, "%")} · Trade {numberLabel(item.team_b.combat.trade.trade_rate, "%")} · Clutch {numberLabel(item.team_b.combat.clutch.win_rate, "%")}</small>}{item.team_b?.utility && <small>Utility dmg/round {numberLabel(item.team_b.utility.utility_damage_per_round)} · HE {numberLabel(item.team_b.utility.he_damage_per_round)} · Fire {numberLabel(item.team_b.utility.fire_damage_per_round)} · Enemies/flash {numberLabel(item.team_b.utility.enemies_flashed_per_flash)} · FA/round {numberLabel(item.team_b.utility.flash_assists_per_round)} · Utility/round {numberLabel(item.team_b.utility.utility_per_round)}</small>}{item.team_b && <em className={`confidence-badge confidence-badge--${item.team_b.confidence_level}`}>{confidenceLabels[item.team_b.confidence_level]}</em>}</span>
