...
class MatchLLMAnalysisService:
    ...
    def __init__(
        ...
        *,
        ...
        self.legacy_explanation_builder = DeterministicMatchExplanationBuilder()
    ...
    async def explain_context(
        ...
        plan: MatchExplanationPlan | MatchExplanationPlanV2 | None = None,
    ) -> MatchLLMAnalysisResponse:
        """v3 pipeline: deterministic analytics first, text-only LLM second."""
        plan = plan or self.explanation_builder.build(context)
        if isinstance(plan, MatchExplanationPlanV2):
            if hasattr(self.provider, "generate_fixed_plan"):
                return await self._explain_fixed_context(context, plan)
            # Compatibility for legacy/test providers. Production Ollama implements
            # generate_fixed_plan and all new persisted runs use plan v2.
            plan = self.legacy_explanation_builder.build(context)
    ...
    async def _explain_fixed_context(
        ...
        for attempt in range(1, self.MAX_ATTEMPTS + 1):
            ...
            return MatchLLMAnalysisResponse(
                context=context, explanation_plan=plan, rendered_analysis=None,
                analysis=normalized_analysis,
                runtime=MatchLLMRuntimeMetadata(
                    provider="ollama", model=self.model,
                    prompt_version="match_analysis_prompt.v5", attempts=attempt,
